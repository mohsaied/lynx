	...
	private void drawConnectivityGraph(Graphics g) {
		...
		mxGraph graph = new mxGraph() {
			public boolean isCellSelectable(Object cell) {
				// disabling drag and drop functionality for edges
		...
		// insert a mxCell vertex for each module in the design
		...
		// right mid, bottom right, bottom mid, left bottom, left mid)
		...
		// draws mxCell port objects inside the module vertices for each bundle
		// that a module has
		...
		// draws connections between bundles in one module to bundles in another
		// module
		for (DesignModule mod : design.getDesignModules().values()) {
		...
		graphComponent.getGraphControl().addMouseListener(new MouseAdapter() {
			public void mouseReleased(MouseEvent e) {
				...
				if (cell != null) {
					...
					if (clickedMod != null) {
						MainPanel.bundleInfo.append("Module Name: " + clickedMod.getName());
						...
						if (clickedMod.getParameters().size() > 0) {
							for (lynx.data.Parameter params : clickedMod.getParameters()) {
					...
					// checking if a bundle has been clicked
					if (clickedBun != null) {
